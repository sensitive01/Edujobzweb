...
const JobsList = () => {
    ...
    const handleAddJob = async (e) => {
        ...
        try {
            ...
            const response = await fetch('https://edujobzbackend.onrender.com/employer/postjob', {
                method: 'POST',
                headers: {
        ...
        try {
            ...
            const updatedData = {
                ...
                skills: Array.isArray(editingJob.skills)
                    ? editingJob.skills
                    : (editingJob.skills || '').split(',').map(skill => skill.trim()).filter(skill => skill),
            ...
            if (!updatedData.jobTitle || !updatedData.companyName || !updatedData.description || !updatedData.location) {
                throw new Error('Please fill all required fields');
            }

            // Validate email format
            if (updatedData.contactEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(updatedData.contactEmail)) {
                throw new Error('Please enter a valid email address');
            }

            const response = await fetch(`https://edujobzbackend.onrender.com/employer/editjob/${editingJob._id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            setFilteredJobs(filteredJobs.map(job => job._id === data._id ? data : job));
    ...
    const openEditModal = (job) => {
        ...
        if (job.applydatetime) {
            ...
            formattedApplyDateTime = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}T${hours.padStart(2, '0')}:${minutes.padStart(2, '0')}`;
        }
    ...
    useEffect(() => {
        if (jobs.length > 0) {
            ...
            // Get salary ranges
            const salaryTypes = [...new Set(jobs.map(j => j.salaryType).filter(Boolean))];
    ...
    const applyFilters = () => {
        ...
        if (selectedStatus !== 'All') {
            if (selectedStatus === 'Active') {
                ...
                filtered = filtered.filter(job => job.postingstatus === selectedStatus);
            }
        }
        ...
        const selectedExpLevels = Object.keys(experienceLevels).filter(level => experienceLevels[level]);
        ...
        // Salary Type filter
        const selectedSalaryRanges = Object.keys(salaryRanges).filter(range => salaryRanges[range]);
        if (selectedSalaryRanges.length > 0) {
            filtered = filtered.filter(job =>
                job.salaryType && selectedSalaryRanges.includes(job.salaryType)
            );
        }
    ...
    const resetFilters = () => {
        ...
        const resetExpLevels = Object.keys(experienceLevels).reduce((acc, level) => {
            acc[level] = false;
            return acc;
        }, {});
        ...
        try {
            if (jobToDelete) {
                ...
                const response = await fetch(
                    `https://edujobzbackend.onrender.com/admin/deletejob/${jobToDelete._id}`,
                    {
                        method: 'DELETE'
                    }
                );
    ...
    const getSalaryDisplay = (job) => {
        if (job.salaryFrom && job.salaryTo) {
            return `₹${parseInt(job.salaryFrom).toLocaleString()} - ₹${parseInt(job.salaryTo).toLocaleString()} ${job.salaryType || ''}`;
        }
    ...
    useEffect(() => {
        applyFilters();
    }, [selectedType, selectedStatus, searchTerm, dateRange, jobTypes, categories, experienceLevels, salaryRanges]);
            ...
            <div className="content">
                ...
                <div className="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
                    <div className="my-auto">
                        <h2>&nbsp; <i className="fa fa-briefcase text-primary"></i> Jobs</h2>
                    </div>
                    <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
                        ...
                        <div className="dropdown me-2">
                            ...
                            <ul className="dropdown-menu dropdown-menu-end p-3">
                                ...
                                {Array.from(new Set(jobs.map(j => j.jobType).filter(Boolean))).map(type => (
                                    <li key={type}>
                                        <button
                                            className="dropdown-item rounded-1"
                            ...
                            <div className="card-body">
                                <div className="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <h3 className="mb-0">{jobs.filter(j => j.isActive === true).length}</h3>
                                    </div>
                            ...
                            <div className="card-body">
                                <div className="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <h3 className="mb-0">{jobs.filter(j => j.postingstatus === 'pending').length}</h3>
                                    </div>
                                    ...
                                    {filteredJobs.length > 0 ? (
                                        filteredJobs.map((job) => (
                                            <tr key={job._id}>
                                                    ...
                                                    <div className="d-flex align-items-center file-name-icon">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="d-flex align-items-center">
                                                                <span className={`badge ${getJobTypeBadge(job.jobType)} me-1`}>
                                                                    {job.jobType || 'N/A'}
                                                                </span>
                                                                {job.category && (
                                                ...
                                                <td>
                                                    <div>
                                                        <h6 className="fw-medium mb-1">{job.companyName || 'N/A'}</h6>
                                                        <small className="text-muted">{job.employerName || 'N/A'}</small>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div>
                                                        <span className="d-block">{job.location || 'N/A'}</span>
                                                        {job.isRemote && (
                                                ...
                                                <td>
                                                    <div className="form-check form-switch">
                                                        <input
                                                            ...
                                                            onChange={() => handleActiveStatusToggle(job._id, job.isActive)}
                                                        />
                    ...
                    <div className="themesettings-inner offcanvas-body">
                        <div className="accordion accordion-customicon1 accordions-items-seperate" id="settingtheme">
                            ...
                            {Object.keys(categories).length > 0 && (
                                <div className="accordion-item">
                                    <h2 className="accordion-header">
                                        <button className="accordion-button text-dark fs-16" type="button" data-bs-toggle="collapse" data-bs-target="#categories" aria-expanded="true">
                                            Categories
                                        </button>
                                    </h2>
                            ...
                            {Object.keys(experienceLevels).length > 0 && (
                                <div className="accordion-item">
                                    <h2 className="accordion-header">
                                        <button className="accordion-button text-dark fs-16" type="button" data-bs-toggle="collapse" data-bs-target="#experienceLevels" aria-expanded="true">
                                            Experience Levels
                                        </button>
                                    </h2>
                                    <div id="experienceLevels" className="accordion-collapse collapse show">
                                        <div className="accordion-body">
                                            <div className="row gx-3">
                                                <div className="form-group">
                                                    <div className="checkbox-limit">
                            ...
                            {Object.keys(salaryRanges).length > 0 && (
                                <div className="accordion-item">
                                    <h2 className="accordion-header">
                                        <button className="accordion-button text-dark fs-16" type="button" data-bs-toggle="collapse" data-bs-target="#salaryRanges" aria-expanded="true">
                                            Salary Types
                                        </button>
                                    </h2>
                                    <div id="salaryRanges" className="accordion-collapse collapse show">
                                        <div className="accordion-body">
                                            <div className="row gx-3">
                                                <div className="form-group">
                                                    <div className="checkbox-limit">
                            ...
                            <div className="accordion-item">
                                <h2 className="accordion-header">
                                    <button className="accordion-button text-dark fs-16" type="button" data-bs-toggle="collapse" data-bs-target="#jobStatus" aria-expanded="true">
                                        Job Status
                                    </button>
                                </h2>
                                <div id="jobStatus" className="accordion-collapse collapse show">
                                    <div className="accordion-body">
                                        <div className="d-flex align-items-center flex-wrap">
                                            ...
                                            <div className="theme-width m-1 me-2">
                                                ...
                                                />
                                                <label htmlFor="activeStatus" className="d-block rounded fs-12">Active</label>
                                            </div>
                                            <div className="theme-width m-1 me-2">
                                                <input
                                                    type="radio"
                                                ...
                                                />
                                                <label htmlFor="inactiveStatus" className="d-block rounded fs-12">Inactive</label>
                                            </div>
                                            <div className="theme-width m-1 me-2">
                                                <input
                                                    type="radio"
                                ...
                                <div className="modal-body">
                                    ...
                                    {jobToDelete && (
                                        <div className="alert alert-warning">
                                            ...
                                            <br />
                                            <strong>Applications:</strong> {jobToDelete.applications?.length || 0}
                                        </div>
                                    )}
                                </div>
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-md-6 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, applydatetime: e.target.value })}
                                                />
                                            </div>
                                            ...
                                            <div className="col-md-4 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, salaryFrom: e.target.value })}
                                                />
                                            </div>
                                            ...
                                            <div className="col-md-4 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, salaryTo: e.target.value })}
                                                />
                                            </div>
                                            ...
                                            <div className="col-md-4 mb-3">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, salaryType: e.target.value })}
                                                >
                                            ...
                                            <div className="col-md-4 mb-3">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, educationLevel: e.target.value })}
                                                >
                                                    <option value="">Select Level</option>
                                            ...
                                            <div className="col-md-6 mb-3">
                                                ...
                                                <div className="form-check form-switch mt-3">
                                                    <input
                                                        ...
                                                        onChange={(e) => setEditingJob({ ...editingJob, isRemote: e.target.checked })}
                                                    />
                                            ...
                                            <div className="col-md-6 mb-3">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, companyUrl: e.target.value })}
                                            ...
                                            <div className="col-md-12 mb-3">
                                                ...
                                                <textarea
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, applicationInstructions: e.target.value })}
                                                />
                                            </div>
                                            ...
                                            <div className="col-md-6 mb-3">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => setEditingJob({ ...editingJob, postingstatus: e.target.value })}
                                                >
