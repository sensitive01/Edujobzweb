...
const [showFilterSidebar, setShowFilterSidebar] = useState(false);
  ...
  const toggleSubscriptionsModal = () => setShowSubscriptionsModal(!showSubscriptionsModal);
...
const applyFilters = () => {
  ...
  if (selectedType !== 'All') {
    filtered = filtered.filter(subscriber => subscriber.plan === selectedType);
  }
...
const resetFilters = () => {
  ...
  const fetchSubscriptions = async (employerId) => {
    ...
    try {
      const response = await fetch(`https://edujobzbackend.onrender.com/admin/fetchplanbyemp/${employerId}`);
      if (!response.ok) {
...
<div className="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
  <div className="my-auto">
    <h2>&nbsp; <i className="fa fa-users text-primary"></i> Subscribers</h2>
  </div>
  <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
    <div className="dropdown me-2">
      <button
    ...
    <div className="dropdown me-2">
      ...
      <ul className="dropdown-menu dropdown-menu-end p-3">
                      ...
                      <td>
                        <div className="action-icon d-inline-flex">
                          ...
                          <button 
                            ...
                            onClick={() => handleViewSubscriptions(subscriber.id)}
                          >
                            View Subscriptions
                          </button>
        ...
        {showSubscriptionsModal && (
          <div className="modal fade show" id="view_subscriptions" style={{ display: 'block', paddingRight: '17px' }}>
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">All Subscriptions</h5>
                  <button type="button" className="btn-close" onClick={toggleSubscriptionsModal}></button>
                </div>
                        ...
                        <tbody>
                          {subscriptionsData.map((subscription, index) => (
                            <tr key={index}>
                              ...
                              <td>
                                <span className={`badge badge-${subscription.isActive ? 'success' : 'danger'}`}>
                ...
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={toggleSubscriptionsModal}>Close</button>
                </div>
              </div>
            </div>
          </div>
