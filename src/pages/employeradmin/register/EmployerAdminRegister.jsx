...
const EmployerAdminRegister = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.employeradminMobile.trim()) {
      ...
      newErrors.employeradminMobile = 'Please enter a valid 10-digit mobile number';
    }
    ...
    if (!formData.employeradminPassword) {
      ...
      newErrors.employeradminPassword = 'Password must be at least 6 characters';
    }

    if (!formData.employerconfirmPassword) {
      newErrors.employerconfirmPassword = 'Please confirm your password';
    } else if (formData.employeradminPassword !== formData.employerconfirmPassword) {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setOtpError(response.data.message || response.data.error || 'User already exists');
      } else {
        setOtpError(response.data.error || 'Failed to send OTP');
      }
    } catch (error) {
      setOtpError(error.response?.data?.error || 'Network error. Please try again.');
    } finally {
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await axios.post(`${VITE_BASE_URL}/employeradmin/verifyemailotp`, {
        userEmail: formData.employeradminEmail,
        otp
      });
    ...
    try {
      ...
      if (errorResponse?.errors) {
        ...
        errorResponse.errors.forEach(error => {
          if (error.path === 'employeradminEmail' || error.path === 'userEmail') {
            fieldErrors.employeradminEmail = 'This email is already registered';
            setApiError('Email address is already registered');
          } else if (error.path === 'employeradminMobile' || error.path === 'userMobile') {
            fieldErrors.employeradminMobile = 'This mobile number is already registered';
                      ...
                      <div className="vh-100 d-flex flex-column justify-content-between p-4 pb-0">
                        ...
                        <div>
                          ...
                          {currentMessage && (
                            <div
                              className={`alert ${
                                currentMessage.type === 'error' ? 'alert-danger' : 'alert-success'
                              } mb-3`}
                              role="alert"
                            >
                              {currentMessage.type === 'success' && registrationSuccess ? (
                          ...
                          {!registrationSuccess ? (
                            <>
                                ...
                                <div className="input-group">
                                  <input
                                    ...
                                    className={`form-control border-end-0 ${
                                      errors.employeradminUsername ? 'is-invalid' : ''
                                    }`}
                                    required
                                  />
                              ...
                              <div className="mb-3">
                                ...
                                <div className="input-group">
                                  <input
                                    ...
                                    onChange={handleChange}
                                    className={`form-control border-end-0 ${
                                      errors.employeradminEmail ? 'is-invalid' : ''
                                    }`}
                                    required
                                    disabled={isOtpSent}
                                  />
                                ...
                                {errors.otp && (
                                  <div className="text-danger small mt-1">{errors.otp}</div>
                                )}
                              </div>
                              ...
                              <div className="mb-3">
                                ...
                                <div className="input-group">
                                  <input
                                    ...
                                    onChange={handleChange}
                                    className={`form-control border-end-0 ${
                                      errors.employeradminMobile ? 'is-invalid' : ''
                                    }`}
                                ...
                                <div className="pass-group" style={{ position: 'relative' }}>
                                  <input
                                    ...
                                    className={`form-control ${
                                      errors.employeradminPassword ? 'is-invalid' : ''
                                    }`}
                              ...
                              <div className="mb-3">
                                ...
                                <div className="pass-group" style={{ position: 'relative' }}>
                                  <input
                                    ...
                                    onChange={handleChange}
                                    className={`form-control ${
                                      errors.employerconfirmPassword ? 'is-invalid' : ''
                                    }`}
                                    style={{ paddingRight: '40px' }}
                                    required
                                  />
                              ...
                              {errors.agreeTerms && (
                                <div className="text-danger mb-3">{errors.agreeTerms}</div>
                              )}
                              ...
                              <div className="mb-3">
                                <button
                                  ...
                                  disabled={loading || !isOtpVerified || !formData.agreeTerms}
                                >
                                  {loading ? (
                                    <>
                            ...
                            <div className="text-center">
                              ...
                              <Link to="/employer-admin/login" className="btn btn-primary">
                                Continue to Login
                              </Link>
                            </div>
                          )}
