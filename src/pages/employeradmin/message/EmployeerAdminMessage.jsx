...
const EmployeerAdminMessage = () => {
      ...
      const enhancedChats = await Promise.all(
        response.data.data.map(async (chat) => {
          try {
            ...
            const unreadResponse = await axios.get(`${VITE_BASE_URL}/employer/unread`, {
              params: {
  ...
  const markMessagesAsRead = async (employeeId, jobId) => {
    try {
      await axios.post(`${VITE_BASE_URL}/employer/mark-read`, {
        employeeId,
        employerId: employerAdminData._id,
        jobId,
        viewer: 'employer'
                            ...
                            <div className="chat-user-info">
                              ...
                              <div className="chat-user-time">
                                ...
                                <div className="chat-pin">
                                  ...
                                  {index % 4 === 0 && <i className="ti ti-heart-filled text-warning"></i>}
                                  {chat.unreadCount > 0 && (
                                    <span className="count-message fs-12 fw-semibold">{chat.unreadCount}</span>
                                  )}
                                </div>
                              </div>
                            </div>
