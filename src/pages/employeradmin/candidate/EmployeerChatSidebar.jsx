...
useEffect(() => {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const newMsg = {
                id: Date.now(),
                sender: 'You',
                avatar: employerAdminData.profilePicture || 'employer/assets/img/profiles/avatar-14.jpg',
                time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                content: newMessage || (file ? `[${file.type.startsWith('image') ? 'Image' : 'Audio'}]` : ''),
                isMe: true,
                ...(file && {
                    mediaUrl: file.type.startsWith('audio') ? URL.createObjectURL(file) :
                        file.type.startsWith('image') ? URL.createObjectURL(file) : null,
                    mediaType: file.type.startsWith('audio') ? 'audio' :
    ...
    const createCalendarEvent = async (eventData) => {
        try {
            ...
            const response = await axios.post(
                'https://edujobzbackend.onrender.com/employer/createcalender',
                eventData,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`,
            ...
            if (response.data.success) {
                ...
                if (selectedStatus === "Interview Scheduled") {
                    try {
                        ...
                        const endDateTime = new Date(interviewDateTime.getTime() + 60 * 60 * 1000); // Add 1 hour
                        ...
                        const eventData = {
                            ...
                            description: statusNotes || `Interview scheduled for ${selectedJobTitle}`,
                            location: location,
    ...
    useEffect(() => {
        if (!recording && audioChunks.length > 0 && !isSendingAudio) {
            ...
            axios.post('https://edujobzbackend.onrender.com/employer/sendchats', formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'multipart/form-data'
                }
                ...
                <form className="footer-form" onSubmit={handleSendMessage}>
                    <div className="chat-footer-wrap" style={footerFormStyles}>
                        <div className="form-item">
                            <button
                                type="button"
                                className={`btn btn-sm rounded-circle ${recording ? 'btn-danger' : 'btn-light'}`}
                                onClick={toggleRecording}
                                style={footerButtonStyles}
                            >
                                <i className={`ti ti-${recording ? 'square' : 'microphone'}`}></i>
                            </button>
                        </div>
