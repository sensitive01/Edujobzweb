...
import { FaCog, FaChevronRight, FaCheckCircle, FaArrowLeft, FaArrowRight, FaSearch } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      try {
        ...
        const response = await axios.get(`https://edujobzbackend.onrender.com/applicant/${userData._id}`);
        ...
        if (err.response?.data?.message === "No applied jobs found") {
          ...
          setError(err.response?.data?.message || err.message || 'Failed to fetch applied jobs');
        }
      } finally {
        setLoading(false);
      }
            ...
            <div className="jobplugin__settings">
              ...
              <div className="jobplugin__settings-content">
                <div className="jobplugin__settings-head">
                  ...
                  <p className="jobplugin__settings-head__text">Your job applications will appear here</p>
                </div>
                ...
                {error ? (
                  ...
                  <>
                    <div className="row justify-content-center">
                      {appliedJobs.map((job) => (
                        <div key={job._id} className="col-12 col-sm-6 col-lg-4 col-xl-4 mb-15 mb-md-30">
                          <article className="featured-category-box border border-secondary pt-20">
                            ...
                            <div className="textbox">
                              ...
                              <address className="location pt-0">
                                ...
                                <span className="text">{job.location || 'Location not specified'}</span>
                              </address>
                              <strong className="h6 text-primary mb-0">{job.title || 'Job Title'}</strong>
                              <span className="subtitle">{job.subject || 'Subject'}</span>
                              <hr />
                              <div className="job-info">
                                ...
                                <span className="subtext">
                                  <b className="text-primary">Posted:</b> {new Date(job.createdAt).toLocaleDateString()}
                                </span>
                              </div>
                              <Link to={`/job-details/${job._id}`} className="btn btn-dark-yellow btn-sm">
                  ...
                  <div className="empty-state text-center py-5 px-3">
                    ...
                    <p className="text-muted mb-4 px-md-5 mx-md-5">
                      You haven't applied to any jobs yet. Browse available positions and apply to get started!
                    </p>
