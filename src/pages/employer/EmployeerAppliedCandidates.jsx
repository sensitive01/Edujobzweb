  ...
  useEffect(() => {
    const fetchJobAndCandidates = async () => {
      try {
        ...
        // First fetch job details to verify it belongs to the employer
        const jobResponse = await fetch(
          `https://edujobzbackend.onrender.com/employer/fetchjob/${jobId}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`
            }
        ...
        if (jobData.employerId !== employerData._id) {
          throw new Error('You are not authorized to view candidates for this job');
        }
        
        setJobDetails(jobData);

        // Then fetch candidates for this specific job
        const candidatesResponse = await fetch(
          `https://edujobzbackend.onrender.com/employer/viewappliedcandidates/${jobId}`,
          {
            headers: {
              'Authorization': `Bearer ${token}`
            }
        ...
        if (!candidatesResponse.ok) {
          throw new Error('Failed to fetch candidates for this job');
        }
        
        const candidatesData = await candidatesResponse.json();
        setCandidates(candidatesData.data || []);
        setFilteredCandidates(candidatesData.data || []);
      } catch (err) {
        console.error('Fetch error:', err);
        setError(err.message);
      } finally {
